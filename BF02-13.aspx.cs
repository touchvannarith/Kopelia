...
{
    public partial class BF02_13 : Page
    {
        ...
        private bool FillValues(int id, int clientId)
        {
            try
            {
                ...
                var data = new JavaScriptSerializer().Deserialize<DataModelBF01_13>(obj.Value);
                txtDossier.Text = data.txtDossier;
                txtDateSignature.Text = data.txtDateDeSignature;
                txtRedacteur.Text = data.txtRedacteur;
                radProcuration1.Checked = data.radProcuration1.TransformToBoolean();
                radProcuration2.Checked = data.radProcuration2.TransformToBoolean();
        ...
        private void SetRange()
        {
            ...
            _excelService.SetRange(_sessionId, "ENTREE_S",
                        new RangeCoordinates() { Row = 55, Column = 5, Height = 5, Width = 2 }, new object[]
                        {
                                new object[] {radProcuration1.Checked.TransformToBooleanFr(), ""},
                                new object[] {radProcuration2.Checked.TransformToBooleanFr(), ""},
        ...
        private void SetValues(DataTable dt)
        {
            ...
            D723.InnerText = dt.Rows[21][(int)ColumnBF02_13.Col1].ToString();
            D724.InnerText = dt.Rows[22][(int)ColumnBF02_13.Col1].ToString();
            D725.InnerText = dt.Rows[23][(int)ColumnBF02_13.Col1].ToString();
            D726.InnerText = dt.Rows[24][(int)ColumnBF02_13.Col1].ToString();
            ...
            html += string.Format("<div><br/><br/></div><div align='right'><img width='50%' src='{0}' id='msg' /></div>",
                Request.PhysicalApplicationPath + "tmp\\BF02-13\\chart.png");
            //DÉTAIL DES FRAIS
            html += "<table border-collapse='collapse' border='0' width='100%' size='1'>";
            ...
            html += "<tr><td bgcolor='#01ABE4' align='center' colspan='4'><b><font color='#FFFFFF' size='11px'>Emoluments du notaire - C.com. Art. A 444-167</font></b></td></tr>";
            html += "<tr><td colspan='4' bgcolor='#01ABE4'></td></tr>";
            D705.InnerText = dt.Rows[3][(int)ColumnBF02_13.Col1].ToString();
            html += string.Format(
            ...
            html += "<tr><td colspan='4' bgcolor='#01ABE4'></td></tr>";
            D709.InnerText = dt.Rows[7][(int)ColumnBF02_13.Col1].ToString();
            D710.InnerText = dt.Rows[8][(int)ColumnBF02_13.Col1].ToString();
            D711.InnerText = dt.Rows[9][(int)ColumnBF02_13.Col1].ToString();
            D712.InnerText = dt.Rows[10][(int)ColumnBF02_13.Col1].ToString();
            D713.InnerText = dt.Rows[11][(int)ColumnBF02_13.Col1].ToString();
            html += string.Format(
            ...
            html += "<tr><td colspan='4' bgcolor='#01ABE4'></td></tr>";
            D717.InnerText = dt.Rows[15][(int)ColumnBF02_13.Col1].ToString();
            html += string.Format(
            ...
            D721.InnerText = dt.Rows[19][(int)ColumnBF02_13.Col1].ToString();
            html += string.Format(
        ...
        protected void btnSynthese_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                watch = System.Diagnostics.Stopwatch.StartNew();
                var data = ExcelServiceHelper.GetRange(_excelService, _sessionId, "SORTIE_S", 701, 3, 25, 1, out _status);
                watch.Stop();
                elapsedMs = watch.ElapsedMilliseconds;
                ...
                CalculezMaintenantDataAccess.Save(Session["CLIENT_ID"].TransformToInt(), "BF02-13");
        ...
        protected void btnEnregistrer_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var data = @"{" +
                           ...
                           "','radProcuration1':'" + radProcuration1.Checked + "','radProcuration2':'" +
                           radProcuration2.Checked +
                ...
                if (obj == null)
                {
                    SimulationActeDataAccess.Save(data, txtLibelle.Text, "BF02-13", false, Session["CLIENT_ID"].TransformToInt());
                }
                else
                {
        ...
        protected void btnEnregistrerSous_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var data = @"{" +
                           ...
                           "','txtDébours':'" + txtDébours.Text +
                           "','chkUtilisation_du_futur_tarif':'" + hdUtilisation_du_futur_tarif.Value + "'}";
                SimulationActeDataAccess.Save(data, txtLibelleSous.Text, "BF02-13", false, Session["CLIENT_ID"].TransformToInt());
                Page.ClientScript.RegisterStartupScript(GetType(), "toastr_message", @"toastr.success('Cette simulation a été enregistrée avec succès.', 'Notification', {timeOut: 5000});", true);
                //BindSimulationList(0);
                //BindSimulationArchiveList(0);
                Page.ClientScript.RegisterStartupScript(GetType(), "input_form_click", "ShowHide_input_form($('#btnSynthese'));", true);
        ...
        protected void btnSendEmail_Click(object sender, EventArgs e)
        {
            try
            {
                PdfHelper.GeneratePdf("BF02-13", hdPiechart.Value, "", 1, hdSaisie.Value, hdResult.Value, out string pdf);
        ...
        {
            var filename = PdfHelper.GeneratePdf("BF02-13", hdPiechart.Value, "", 1, hdSaisie.Value, hdResult.Value, out string pdf);
